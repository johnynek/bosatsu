...
import SeqPattern0.{Cat, Empty}

import cats.implicits._

object StringSeqPattern0Gen {
  ...
  val genPat: Gen[SeqPattern0[Char]] = {
  ...
  implicit val arbPattern: Arbitrary[SeqPattern0[Char]] = Arbitrary(genPat)

  implicit lazy val shrinkPat: Shrink[SeqPattern0[Char]] =
    Shrink {
  ...
  def unany[A](p: SeqPattern0[A]): SeqPattern0[A] =
    p match {
      case Empty => Empty
  ...
  def unwild[A](p: SeqPattern0[A]): SeqPattern0[A] =
    p match {
      case Empty => Empty
...
abstract class SeqPattern0Laws[E, I, S, R] extends FunSuite {
  import SeqPart.{Wildcard, AnyElem, Lit}

  type Pattern = SeqPattern0[E]
  val Pattern = SeqPattern0
...
class StringSeqPattern0SetLaws extends SetOpsLaws[SeqPattern0[Char]] {
  type Pattern = SeqPattern0[Char]
  val Pattern = SeqPattern0
  ...
  def genItem: Gen[Pattern] = StringSeqPattern0Gen.genPat.map { p =>
  ...
  def eqUnion: Gen[Eq[List[Pattern]]] =
    Gen.listOfN(100, StringSeqPattern0Gen.genBitString).map { tests =>
      new Eq[List[Pattern]] {
...
class SeqPattern0Test extends SeqPattern0Laws[Char, Char, String, Unit] {
  import SeqPart._

  def genPattern = StringSeqPattern0Gen.genPat
  def genNamed = StringSeqPattern0Gen.genNamed
  def genSeq = StringSeqPattern0Gen.genBitString
