...
object Generators {
  ...
  val opGen: Gen[Identifier.Operator] = {
    ...
    lazy val multi: Gen[Identifier.Operator] =
      for {
        ...
        asStr = ms.mkString
        res <-
          (if (asStr != "<-") Gen.const(Identifier.Operator(asStr)) else multi)
      } yield res
  ...
  val genStruct: Gen[Statement] =
    Gen
      .zip(constructorGen, Gen.listOf(typeRefVarGen))
      .map { case ((name, args), ta) =>
        Statement.Struct(name, NonEmptyList.fromList(ta.distinct), args)(
          emptyRegion
        )
      }
  ...
  val genEnum: Gen[Statement] =
    ...
    } yield Statement.Enum(name, NonEmptyList.fromList(ta.distinct), cons)(
      emptyRegion
    )
  ...
  val interfaceGen: Gen[Package.Interface] =
    for {
      ...
      te <- typeEnvGen(
        p,
        Gen.oneOf(Variance.co, Variance.phantom, Variance.contra, Variance.in)
      )
  ...
  def genTypedExpr[A](
    ...
    if (depth <= 0) lit
      ...
      val genGeneric =
        Gen
          .zip(Generators.nonEmpty(NTypeGen.genBound), recurse)
          .map { case (vs, t) => TypedExpr.Generic(vs, t) }

      val ann =
        Gen
          .zip(recurse, typeGen, genTag)
  ...
  def genOnePackage[A](
    ...
    def impFromExp(
        exp: List[(Package.Interface, ExportedName[Referant[Variance]])]
    ): Gen[List[Import[Package.Interface, NonEmptyList[Referant[Variance]]]]] =
      exp
        .groupBy(_._1)
        .toList
        .traverse { case (p, exps) =>
          val genImps
              : Gen[List[ImportedName[NonEmptyList[Referant[Variance]]]]] =
            exps
              .groupBy(_._2.name)
              .iterator
              .toList
    ...
    val genImports: Gen[
      List[Import[Package.Interface, NonEmptyList[Referant[Variance]]]]
    ] =
    ...
    def definedTypesFromImp(
        i: Import[Package.Interface, NonEmptyList[Referant[Variance]]]
    ...
    ): StateT[Gen, (rankn.TypeEnv[Variance], Set[Identifier.Bindable]), Unit] =
      StateT
        .get[Gen, (rankn.TypeEnv[Variance], Set[Identifier.Bindable])]
          ...
          StateT
            ...
            .flatMap {
              case 0 =>
                ...
                StateT.pure[
                  Gen,
                  (rankn.TypeEnv[Variance], Set[Identifier.Bindable]),
                  Unit
                ](())
              case _ =>
                // add something:
    ...
    def genProg(
        pn: PackageName,
        imps: List[Import[Package.Interface, NonEmptyList[Referant[Variance]]]]
    ): Gen[Program[rankn.TypeEnv[Variance], TypedExpr[A], Any]] =
      genTypeEnv(pn, imps)
    ...
    def genExports(
        pn: PackageName,
        p: Program[rankn.TypeEnv[Variance], TypedExpr[A], Any]
    ): Gen[List[ExportedName[Referant[Variance]]]] = {
