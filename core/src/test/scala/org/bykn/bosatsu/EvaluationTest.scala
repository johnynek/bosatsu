...
main = a"""
    ...
    ) { case PackageError.UnknownImportName(_, _, _, _) => () }
...
""")) { case te @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package B, unknown constructor X1\nRegion(49,50)"
      )
      ()
    }
...
""")) { case te @ PackageError.RecursionError(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, recur but no recursive call to fn\nRegion(25,42)\n"
      )
      ()
    }
...
""")) { case te @ PackageError.RecursionError(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, recur not on an argument to the def of fn, args: x\nRegion(25,43)\n"
      )
      ()
    }
...
""")) { case te @ PackageError.RecursionError(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, unexpected recur: may only appear unnested inside a def\nRegion(47,70)\n"
      )
      ()
    }
...
""")) { case te @ PackageError.RecursionError(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, illegal shadowing on: fn. Recursive shadowing of def names disallowed\nRegion(25,81)\n"
      )
      ()
    }
...
""")) { case te @ PackageError.RecursionError(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, invalid recursion on fn\nRegion(63,79)\n"
      )
      ()
    }
...
""")) { case te @ PackageError.TypeErrorIn(_, _) =>
      assert(
        te.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, type error: expected type Bosatsu/Predef::Int to be the same as type ?2 -> ?3\nhint: this often happens when you apply the wrong number of arguments to a function.\nRegion(73,84)"
      )
      ()
    }
...
""")) { case s @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        s.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, bind names foo shadow external def\nRegion(57,71)"
      )
      ()
    }
...
""")) { case s @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        s.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package A, external def: foo defined multiple times\nRegion(21,55)"
      )
      ()
    }
  }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, Foo found declared: [a], not a superset of [anon0]\nRegion(14,30)"
      )
      ()
    }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, Foo found declared: [a], not a superset of [a, b]\nRegion(14,39)"
      )
      ()
    }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, Enum found declared: [a], not a superset of [anon0]\nRegion(14,34)"
      )
      ()
    }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, Enum found declared: [a], not a superset of [a, b]\nRegion(14,48)"
      )
      ()
    }
  }
...
""")) { case sce @ PackageError.DuplicatedImport(_) =>
  ...
  test("test duplicate package message") {
    ...
    evalFail(List(pack, pack)) {
      case sce @ PackageError.DuplicatedPackageError(_, _) =>
        assert(
          sce.message(
            Map.empty,
            Colorize.None
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, type name: Foo defined multiple times\nRegion(14,24)"
      )
      ()
    }
  }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Err, constructor: Foo defined multiple times\nRegion(14,27)"
      )
      ()
    }
  }
...
""")) { case re @ PackageError.RecursionError(_, _) =>
      assert(
        re.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package S, recur not on an argument to the def of bar, args: y, _: String, x\nRegion(107,165)\n"
      )
      ()
    }
  }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Foo, repeated bindings in pattern: a\nRegion(48,49)"
      )
      ()
    }
    evalFail(List("""
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Foo, unknown type: Either\nRegion(14,50)"
      )
      ()
    }
  }
...
""")) { case sce @ PackageError.SourceConverterErrorIn(_, _) =>
      assert(
        sce.message(
          Map.empty,
          Colorize.None
        ) == "in file: <unknown source>, package Foo, and_again found declared types: [b], not a subset of [a]\nRegion(71,118)"
      )
      ()
    }
  }
