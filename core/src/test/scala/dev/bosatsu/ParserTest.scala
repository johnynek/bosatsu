...
"""
  ...
  test("external type spellings round trip to the same canonical statement") {
    forAll(Generators.genExternalStruct) { stmt =>
      ...
      assertEquals(
        Document[Statement].document(preferred).render(80),
        canonical
      )
      assertEquals(
        Document[Statement].document(compatible).render(80),
        canonical
      )
    }
  }
  ...
  test(
    "external type leaves values named type and transparent aliases distinct"
  ) {
    val parser = Statement.parser1.map(_.replaceRegions(emptyRegion))
