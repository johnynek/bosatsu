    ...
    (stripTypeWrappers(arg), asBoolSelector(branches)) match {
      case (inner @ Match(innerArg, innerBranches, _), Some((ifTrue, ifFalse))) =>
        innerBranches.traverse { inner =>
          ...
          Match(innerArg, mapped, tag, inner.matchKind)
        }
      case _ =>
        None
    }
    ...
    te match {
          ...
          e1 match {
            ...
            case m @ Match(arg1, branches, tag1)
              ...
              b1 match {
                ...
                case Some(bs) =>
                  val m1 = Match(arg1, bs, tag1, m.matchKind)
                ...
                case None =>
                  in1 match {
                    case m @ Match(marg, branches, mtag)
                      ...
                      normalize1(
                        namerec,
                        Match(marg, b1, mtag, m.matchKind),
                        scope,
                        typeEnv
                      )
                    case _ =>
      ...
      case m @ Match(arg, branches, tag) =>
          ...
          bs.toList match {
            case Branch(p, Some(g), e1) :: tail
              ...
              NonEmptyList.fromList(tail).map { tailNel =>
                val fallback = Match(arg1, tailNel, tag, m.matchKind)
                Match(
                  g,
                  NonEmptyList.of(
        ...
        rewriteLeadingWildcardGuard(a1, branches1a) match {
          ...
          case None =>
            if (changed1 == 0) {
              val m1 = Match(a1, branches, tag, m.matchKind)
              ...
              normalize1(namerec, Match(a1, branches1a, tag, m.matchKind), scope, typeEnv)
            }
        }
    }
  }
