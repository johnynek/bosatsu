...
object ProtoConverter {
  ...
  object ProtoState {
    ...
    given MonadError[ProtoState, Throwable] with
      ...
      def flatMap[A, B](fa: ProtoState[A])(
          f: A => ProtoState[B]
      ): ProtoState[B] =
        fa.flatMap(f)
    ...
    ReaderT[Try, DecodeState, Array[TypedExpr[Region]]] { ds =>
      def expressionFromProto(
          ...
          qev match {
            ...
            case Some(proto.QuantifierEvidence(src, dst, fs, exs, _)) =>
              (
                ...
                (sourceAtSolve, targetAtSolve, forallSolved, existsHidden) =>
                  Some(
        ...
        ex.value match {
          ...
          case Value.MatchExpr(
                proto.MatchExpr(argId, branches, matchKind0, _)
              ) =>
            def decodeGuard(
                b: proto.Branch
            ): Try[Option[TypedExpr.BranchGuard[Region]]] = {
            ...
            def buildBranch(b: proto.Branch): Try[
              ...
              (
                ...
                TypedExpr.Branch.fromGuardNode(pattern, guardNode, expr)(using
                  patternRegion
                )
              )
            ...
            case m @ Match(argE, branches, _) =>
              def encodeGuard(
                ...
                guard match {
                  ...
                  case guard @ TypedExpr.MatchGuard(
                        argExpr,
                        pattern,
                        guardExpr
                      ) =>
                      ...
                      (
                        ...
                        Some(
                          proto.MatchGuard(
                            ...
                            patternRegion =
                              Some(regionToProto(guard.patternRegion))
                          )
                        )
                      )
                    }
              ...
              recurse(argE)
                ...
                .flatMap { case (argId, branches) =>
                  val ex = proto.MatchExpr(
                    argId,
                    branches,
                    matchKindToProto(m.matchKind)
                  )
  ...
  def expNameToProto[V](
    ...
    val protoRef: Tab[proto.Referant] =
      referantToProto(allDts, allAliases, e.tag)
  ...
  def interfaceToProto(iface: Package.Interface): Try[proto.Interface] = {
    ...
    val tryExports =
      iface.exports.traverse(expNameToProto(allDts, allAliases, _))

    val packageId = getId(iface.name.asString)

    val last = packageId
      .product(tryProtoDts)
      .product(tryProtoAliases)
      .product(tryExports)
  ...
  def importToProto(
    ...
    for {
      ...
      imps <- i.items.toList.traverse(
        importedNameToProto(allDts, allAliases, _)
      )
  ...
  def packageToProto[A: HasRegion](
      cpack: Package.Typed[A]
  ): Try[proto.Package] = {
    ...
    ReaderT
      ...
      .map { ds =>
        ...
        val te0: TypeEnv[Kind.Arg] =
          TypeEnv.fromDefinitionsAndAliases(
            ds.getDefinedTypes,
            ds.getTypeAliases
          )
