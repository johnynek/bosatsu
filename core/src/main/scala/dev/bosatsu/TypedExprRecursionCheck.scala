...
object TypedExprRecursionCheck {
  ...
  private object Impl {
    ...
    object SmtBranchState {
      val Empty: SmtBranchState =
        SmtBranchState(Map.empty, Map.empty, Map.empty, Map.empty, Vector.empty, 0)
    }
          ...
          group.iterator.zipWithIndex.flatMap { case (item, idx) =>
            val maybeTypedName =
              typedArgs
                ...
                .map(_._1)
            item.topNames.iterator.flatMap { topName =>
              val bindName = maybeTypedName.getOrElse(topName)
              List(topName -> RecurTargetItem(gidx, idx, bindName))
            }
          }
        }
        .toMap
    ...
    private def isIntType(tpe: Type): Boolean =
      Type.normalize(tpe) == Type.IntType

    private def isBoolType(tpe: Type): Boolean =
      Type.normalize(tpe) == Type.BoolType

    private def isComparisonType(tpe: Type): Boolean =
      Type.normalize(tpe) == comparisonType
        ...
        case TypedExpr.App(fn, args, _, _) =>
          predefFnName(fn) match {
            ...
            case Some(Identifier.Name("div")) =>
              lowerBinaryInt(args, state)(SmtExpr.Div(_, _))
            case Some(Identifier.Name("mod_Int")) =>
              lowerBinaryInt(args, state)(SmtExpr.Mod(_, _))
            case _ =>
              (None, state)
          }
    ...
    private def lowerPatternCondition(
      ...
      pattern match {
        ...
        case Pattern.Union(head, rest) =>
          val all = (head :: rest.toList).foldLeft(
          ...
          if (all._3) (Some(mkOr(all._1.reverse.toVector)), all._2)
          else (None, all._2)
    ...
    private def initBranchSmtState(inrec: InDefRecurred): SmtBranchState =
      inrec.inRec.typedArgs.iterator.flatMap(_.iterator).foldLeft(SmtBranchState.Empty) {
        case (state, (name, tpe)) =>
    ...
    private def lowerAsIntOrFail(
      ...
      argOpt match {
        ...
        case None          =>
          val detail =
            Some(
              s"unable to lower recursive argument ${arg.reprString} to SMT Int expression"
            )
          (
            None,
            state1,
    ...
    private def recurAllowedByLexOrder(
      ...
      getSt.flatMap {
        case InRecurBranch(_, branch, allowedNow, namesNow, smtState0) =>
          ...
          var accepted = false
          val it = target.iterator
            .zip(allowedPerTarget.iterator)
            .zip(callArgsByTarget.iterator)

          while (it.hasNext && immediateError.isEmpty && !accepted) {
            val ((targetItem, allowed), argExpr) = it.next()
            val step =
              if (isIntType(targetItemType(inrec, targetItem))) {
                ...
                smtState = smt1
                intStep
              } else classifyStructuralArg(targetItem, allowed, argExpr)
          ...
          setSt(
            InRecurBranch(inrec, branch, allowedNow, namesNow, smtState)
    ...
    private def withTemporaryRecurBranchNames[A](
      ...
      getSt.flatMap {
        case start @ InRecurBranch(inrec, branch, allowed, names, smtState) =>
          val (allowed1, names1) = update(allowed, names)
          (
            setSt(InRecurBranch(inrec, branch, allowed1, names1, smtState)) *> in,
            getSt
          )
            .flatMapN {
              case (a, InRecurBranch(ir1, b1, _, _, smt1)) =>
                setSt(InRecurBranch(ir1, b1, allowed, names, smt1)).as(a)
              // $COVERAGE-OFF$ this should be unreachable
              case (_, unexpected) =>
                sys.error(
    ...
    private def withTemporaryRecurBranchSmtState[A](
      ...
      getSt.flatMap {
        case start @ InRecurBranch(inrec, branch, allowed, names, smtState) =>
          ...
          (
            setSt(InRecurBranch(inrec, branch, allowed, names, smtState1)) *> in,
            getSt
          ).flatMapN {
            case (a, InRecurBranch(ir1, b1, allowed1, names1, _)) =>
              setSt(InRecurBranch(ir1, b1, allowed1, names1, smtState)).as(a)
            // $COVERAGE-OFF$ this should be unreachable
            case (_, unexpected) =>
              sys.error(
                s"invariant violation expected InRecurBranch: start = $start, end = $unexpected"
    ...
    private def checkApply(
      ...
      getSt.flatMap {
        ...
        case irb @ InRecurBranch(inrec, _, allowedPerTarget, names, _) =>
          ...
          recurTag(currentPackage, arg, tag) match {
            ...
            case Some(recur) =>
              ...
              getSt.flatMap {
                ...
                case InDef(_, defname, typedArgs, sourceArgs, _, locals) =>
                  toSt(getRecurTarget(defname, sourceArgs, typedArgs, recur, locals)).flatMap {
                    target =>
                      ...
                      val endBranch: St[Unit] =
                        getSt.flatMap {
                          case InRecurBranch(irr, _, _, _, _) => setSt(irr)
                          case illegal                  =>
                            // $COVERAGE-OFF$ this should be unreachable
