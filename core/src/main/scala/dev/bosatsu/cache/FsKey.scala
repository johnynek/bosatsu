...
/** Filesystem cache key plus decode context.
  ...
  *   - `depInterfaceHashes` are part of cache identity and are what
  *     `keyHashValue` hashes for stable key lookup. `CompileCache.generateKey`
  *     computes them from the provided dependency interfaces.
  *   - `depInterfaces` are required when reading cached package bytes back,
  *     because package decoding needs dependency interfaces in scope.
  *
  * Other fields are also part of identity:
  *   - package/mode/options/compiler/phase/schema ensure we do not mix
  *     artifacts across incompatible compiler runs.
