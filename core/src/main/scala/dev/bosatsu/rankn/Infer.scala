        ...
        outArgs <- GetEnv.map { env =>
          if (env.aliases.isEmpty) a1s
            ...
            a1s.zip(a2s).map { case (a1, a2) =>
              val same =
                env
                  .normalizeAliasesDeep(a1)
                  .sameAs(env.normalizeAliasesDeep(a2))
              if (same) a2 else a1
            }
        }
        ...
        normalizeAliasPair(t, rho).flatMap { case (t1, rho1) =>
          if ((t1 ne t) || (rho1 ne rho)) {
            ...
            (t, rho) match {
              ...
              case (
                    rho1: (Type.Leaf | Type.TyApply),
                    ta @ Type.TyApply(l2, r2)
                  ) =>
                for {
              ...
              case (ta @ Type.TyApply(l1, r1), rho2: Type.Leaf) =>
                ...
                for {
                  ...
                  _ <- varianceOfConsKind(Type.TyApply(l2, r2), kl, right)
                    .flatMap {
                      case Variance.Covariant =>
                        subsCheck(r1, r2, left, right, direction)
                      case Variance.Contravariant =>
              ...
              case Type.Fun.SimpleUniversal(univ, inT, outT)
                ...
                Type
                  ...
                  .flatMap { instantiation =>
                    ...
                    if (
                      instantiation.subs.nonEmpty && instantiation.frees.isEmpty
                    ) {
            ...
            validKinds.parProductR {
              val remainingFree =
                NonEmptyList.fromList(
                  instantiation.frees.iterator.map { case (_, (k, b)) =>
                    (b, k)
                  }.toList
                )
            ...
            case Type.Fun.SimpleUniversal(us, fnArgTypes, resT)
              ...
              infArgs.sequence
                .flatMap { appliedArgsTE =>
                  ...
                  val noshadows =
                    Type
                      .freeBoundTyVars(resT :: appliedArgTypes0.toList)
                      .toSet ++
                  ...
                  val (appliedToVars, appliedArgShapes) =
                    appliedArgTypes.toList.foldLeft(
                      ...
                      val (argExists, argNoExists) =
                        Type.liftExistentials(argTy)
        ...
        resBranches <- withIdx.parTraverse { case (te, (branch, tpe, idx)) =>
          if (idx != maxIdx) {
            ...
            subsCheckRho2(
              ...
              .map { coerce =>
                branch.copyNode(branch.pattern, branch.guardNode, coerce(te))(
                  using branch.patternRegion
                )
              }
          } else pure(branch)
        ...
        case Some(
          ...
          extendEnvList(outerBindings) {
            inferSigma(argExpr).flatMap { targ =>
              skolemizeExistsOnly(targ.getType).flatMap {
                case (_, scrutineeType) =>
                  typeCheckPattern(
                    pattern,
                    ...
                    for {
                      ...
                      _ <- wholeGuardCheckExpr
                        .traverse(checkRho(_, Type.BoolType))
                        .void
                    } yield {
                      val typedGuard =
      ...
      def maybeWidenNamedBindingType(
        ...
        if (!singletonParametricPattern(innerPattern)) pure(expectedType)
          ...
          if (freeBounds.isEmpty && freeMetas.isEmpty && freeSkolems.isEmpty)
            ...
            val usedBounds0 =
              (Type.tyVarBinders(
                expectedType :: innerBindingTypes
              ) ++ freeBounds).toSet
            val alignedMetas =
              Type.alignBinders(freeMetas, usedBounds0.contains)
