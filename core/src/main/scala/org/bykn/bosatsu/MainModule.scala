...
abstract class MainModule[IO[_]](implicit
  ...
  object MainCommand {
    ...
    case class ToJson(
      ...
      def run =
        Evaluate(inputs, mainPackage, deps, errColor, packRes).runEval
          .flatMap { case (ev, res) =>
        ...
        withEC { implicit ec =>
          for {
            ...
            ins1 <- NonEmptyList
              .fromList(ins)
              .fold(
                moduleIOMonad.raiseError[NonEmptyList[Path]](
                  new Exception("no source files found")
                )
              )(moduleIOMonad.pure(_))
        ...
        tests.read
          ...
          .flatMap { case (testPaths, dependencies) =>
            ...
            if (tests1.isEmpty && dependencies.isEmpty) {
              moduleIOMonad.raiseError(
                new Exception("no test sources or test dependencies")
              )
            } else {
              val typeChecked =
                buildPackMap(tests1, dependencies, errColor, packRes)

              val withTestPackNames = typeChecked
                .flatMap { case (packs, nameMap) =>
                  testPacks
                    .traverse(_.getMain(nameMap))
                    .map {
                      (testPackNames: List[
                        (PackageName, Option[Identifier])
                      ]) =>
