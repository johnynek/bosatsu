      ...
      te match {
        case Generic(params, expr) =>
          val pstr = Doc.intercalate(
            ...
            params.toList.map { p => Doc.text(p.name) }
          )
...
object TypedExpr {
  ...
  case class Generic[T](
      typeVars: NonEmptyList[Type.Var.Bound],
      in: TypedExpr[T]
  ) extends TypedExpr[T] {
    def tag: T = in.tag
  }
  case class Annotation[T](term: TypedExpr[T], coerce: Type, tag: T)
      extends TypedExpr[T]
      ...
      override def map[A, B](te: TypedExpr[A])(fn: A => B): TypedExpr[B] =
        te match {
          ...
          case Annotation(term, tpe, tag) =>
            Annotation(map(term)(fn), tpe, fn(tag))
      ...
      def apply[A](expr: TypedExpr[A]) = {
        expr match {
          ...
          case _ =>
            ...
            val result1 = cores(
              App(expr, coarg(Local(name, arg, expr.tag)), result, expr.tag)
            )
  ...
  def forAll[A](
      params: NonEmptyList[Type.Var.Bound],
      expr: TypedExpr[A]
  ): TypedExpr[A] =
    expr match {
      case Generic(ps, ex0) =>
