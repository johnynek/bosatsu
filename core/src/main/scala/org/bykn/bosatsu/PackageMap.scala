...
object PackageMap {
        ...
        val edeps = ReaderT
          ...
          .flatMapF {
            case nonE @ (h :: tail) if nonE.contains(p.name) =>
              Left(
                NonEmptyList
                  .of(PackageError.CircularDependency(p, NonEmptyList(h, tail)))
              )
            case _ =>
        ...
        case Some(nenu) =>
          Ior.left(
            NonEmptyList.one[PackageError](
              PackageError.DuplicatedPackageError[A](nenu, Show[A].show(_))
            )
          )
        case None =>
          Ior.right(())
  ...
  def inferAll(
    ...
    val infer0: ResolvedU => Par.F[
      Ior[NonEmptyList[PackageError], (TypeEnv[Variance], Package.Inferred)]
    ] =
      Memoize.memoizeDagFuture[ResolvedU, Ior[NonEmptyList[
        PackageError
      ], (TypeEnv[Variance], Package.Inferred)]] {
        ...
        case (Package(nm, imports, exports, (stmt, _)), recurse) =>
           ...
           */
          type ImpRes =
            Import[Package.Interface, NonEmptyList[Referant[Variance]]]
          def stepImport(
