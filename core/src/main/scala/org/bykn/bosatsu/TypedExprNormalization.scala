...
object TypedExprNormalization {
  ...
  def normalizeProgram[A](
      ...
      prog: Program[TypeEnv[Variance], TypedExpr[Declaration], A]
  ): Program[TypeEnv[Variance], TypedExpr[Declaration], A] = {
      ...
      case App(fn, arg, tpe, tag) =>
        ...
        f1 match {
          case ws.ResolveToLambda(b, ltpe, expr, ltag) =>
            // (\y -> z)(x) = let y = x in z
            val a2 =
              if (ltpe != arg.getType) Annotation(arg, ltpe, ltag) else arg
            val expr2 =
              if (tpe != expr.getType) Annotation(expr, tpe, expr.tag) else expr
