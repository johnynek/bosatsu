...
class TypeEnv[+A] private (
    ...
    protected val constructors: SortedMap[
      ...
      (List[(Bindable, Type)], DefinedType[A], Type)
    ],
  ...
  def getConstructor(
      p: PackageName,
      c: Constructor
  ): Option[(List[(Bindable, Type)], DefinedType[A], Type)] =
  ...
  def localValuesOf(p: PackageName): SortedMap[Identifier, Type] =
    (SortedMap.newBuilder[Identifier, Type] ++= values.iterator.collect {
      case ((pn, n), v) if pn == p => (n, v)
    }).result()
      ...
      dt.constructors
        .foldLeft(
          constructors: SortedMap[
            (PackageName, Constructor),
            (List[(Bindable, Type)], DefinedType[A1], Type)
          ]
        ) { case (cons0, cf) =>
  ...
  lazy val typeConstructors: SortedMap[
    (PackageName, Constructor),
    (List[(Type.Var, A)], List[Type], Type.Const.Defined)
  ] =
    constructors.map { case (pc, (params, dt, _)) =>
      (pc, (dt.annotatedTypeParams, params.map(_._2), dt.toTypeConst))
    }
...
object TypeEnv {
  val empty: TypeEnv[Nothing] =
    new TypeEnv(
      ...
      SortedMap.empty[
        (PackageName, Constructor),
        (List[(Bindable, Type)], DefinedType[Nothing], Type)
      ],
