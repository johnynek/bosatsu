...
object Infer {
  ...
  private object Impl {
        ...
        ((t, rho) match {
          case (rho1, Type.Fun(a2, r2)) =>
            ...
            for {
              ...
              rhor2 <- assertRho(
                r2,
                s"subsCheckRho2($t, $rho, $left, $right), line 521",
                right
              )
                ...
                kindOf(nonMeta, right)
                  .flatMap { nmk =>
                    if (Kind.leftSubsumesRight(m.kind, nmk)) {
                      ...
                      fail(
                        Error.KindMetaMismatch(ty1, nonMeta, nmk, left, right)
                      )
                    }
                  }
              }
    ...
    def subsUpper[F[_], G[_]: Functor](
        ...
        onErr: NonEmptyList[Type.Var.Skolem] => Error
          ...
          case Some(nel) =>
            envTpes
              ...
              .flatMap { escTvs =>
                ...
                val badList =
                  if (escTvs.isEmpty) Nil else skolTvs.filter(escTvs)
          ...
          expect match {
            ...
            case infer @ Expected.Inf(_) =>
              for {
                ...
                typedBodyTpe <- extendEnvList(nameVarsT.toList)(
                  inferRho(result)
                )
    ...
    def checkSigma[A: HasRegion](t: Expr[A], tpe: Type): Infer[TypedExpr[A]] = {
      ...
      for {
        checkRho <- subsUpper[Lambda[x => (Expr[x], HasRegion[x])], Infer](
          tpe,
          regionT,
          envTail
        ) { rho =>
  ...
  def typeCheckLets[A: HasRegion](
    ...
    def run(
      ...
      groups match {
        ...
        case group :: tail =>
          group
            .parTraverse { case (name, rec, expr) =>
            ...
            .flatMap { groupChain =>
              ...
              extendEnvListPack(
                pack,
                glist.map { case (b, _, te) => (b, te.getType) }
              ) {
                run(tail)
              }
                .map(glist ::: _)
