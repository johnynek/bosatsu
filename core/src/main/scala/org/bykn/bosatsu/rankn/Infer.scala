...
object Infer {
  ...
  object Env {
    def apply(
      ...
      val variances = typeCons.iterator.map { case (_, (vs, _, c)) =>
        (c, vs.map(_._2))
      }.toMap
  ...
  object Error {
    ...
    case class SubsumptionCheckFailure(
      ...
      def message = {
        ...
        def tStr(t: Type): String =
          Type.fullyResolvedDocument.document(t).render(80)
        s"subsumption check failed: ${tStr(inferred)} ${tStr(
            declared
          )}, bad types: ${bads.map(tStr).toList.mkString(", ")}"
      }
    }
    ...
    case class UnexpectedBound(
      ...
      def message =
        s"unexpected bound ${v.name} at $rb in unification with $in at $rt"
    }
    ...
    case class UnionPatternBindMismatch(
        pattern: Pattern,
        names: List[List[Identifier.Bindable]]
    ) extends NameError {
      def message =
        s"$pattern doesn't bind the same names in all union branches: $names"
    }
    ...
    case class UnexpectedMeta(
      ...
      def message =
        s"meta $m at $left occurs in $in at $right and should not. This implies an infinite type."
    }

    // This is a logic error which should never happen
    case class InferIncomplete(method: String, term: Expr[_])
        extends InternalError {
  ...
  private object Impl {
    ...
    case class GetDataCons(fqn: (PackageName, Constructor), reg: Region)
        extends Infer[Cons] {
      def run(env: Env) =
        RefSpace.pure(env.typeCons.get(fqn) match {
          case None =>
    ...
    case object GetVarianceMap
        extends Infer[Map[Type.Const.Defined, List[Variance]]] {
    ...
    def varianceOf(t: Type): Infer[Option[Variance]] = {
      import Type._
      def variances(
          vs: Map[Type.Const.Defined, List[Variance]],
          t: Type
    ...
    def subsCheckRho(
      ...
      (t, rho) match {
        ...
        case (rho1: Type.Rho, Type.TyApply(l2, r2)) =>
          unifyTyApp(rho1, left, right).flatMap { case (l1, r1) =>
            ...
            (subsCheck(l1, l2, left, right) *> check2).as(
              TypedExpr.coerceRho(rho1)
            )
          }
        case (ta @ Type.TyApply(l1, r1), rho2) =>
          unifyTyApp(rho2, left, right).flatMap { case (l2, r2) =>
            val check2 = varianceOf2(l1, l2).flatMap {
              case Some(Variance.Covariant) =>
                subsCheck(r1, r2, left, right).void
              case Some(Variance.Contravariant) =>
            ...
            // should we coerce to t2? Seems like... but copying previous code
            (subsCheck(l1, l2, left, right) *> check2).as(
              TypedExpr.coerceRho(ta)
            )
          }
        case (t1: Type.Rho, t2) =>
          // rule: MONO
          unify(t1, t2, left, right).as(
            TypedExpr.coerceRho(t1)
          ...
          expect match {
            case Expected.Check((resT, _)) =>
              for {
                ...
                tbranches <- branches.traverse { case (p, r) =>
                  ...
                  checkBranch(
                    p,
                    Expected.Check((tsigma.getType, region(term))),
                    r,
                    resT
                  )
                }
            ...
            case infer @ Expected.Inf(_) =>
              for {
                ...
                resT = tbranches.map { case (_, te) =>
                  (te.getType, region(te))
                }
                ...
                resTRho <- assertRho(
                  resT.head._1,
                  s"infer on match ${tbranches.head}"
                )
        ...
        case (v0 :: vs, consParams, tpeName) =>
          ...
          vars
            ...
            .map { vars1T =>
              ...
              val res = vars1T.foldLeft(Type.TyConst(tpeName): Type.Tau)(
                Type.TyApply(_, _)
              )
              (params1, res)
            }
  ...
  def recursiveTypeCheck[A: HasRegion](
      name: Bindable,
      expr: Expr[A]
  ): Infer[TypedExpr[A]] =
  ...
  def typeCheckLets[A: HasRegion](
    ...
    ls match {
      ...
      case (nm, rec, expr) :: tail =>
        for {
          te <-
            if (rec.isRecursive) recursiveTypeCheck(nm, expr)
            else typeCheck(expr)
