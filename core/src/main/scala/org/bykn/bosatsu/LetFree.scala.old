  ...
  def asString: String = {
    ...
    this match {
      case LetFreeExpression.App(fn, arg) =>
        s"App(${fn.asString},${arg.asString})"
      case LetFreeExpression.ExternalVar(pack, defName, tpe) =>
        s"ExternalVar('${escapeString(pack.asString)}','${escapeString(defName.asString)}', '${escapeString(
          TypeRef.fromTypes(None, tpe :: Nil).apply(tpe).toDoc.render(100)
        )}')"
      case LetFreeExpression.Match(arg, branches) => {
        val serBranches = branches.toList
          .map { case (lfp, lfe) => s"${lfp.asString},${lfe.asString}" }
          .mkString(",")
      ...
      case LetFreeExpression.Struct(enum, args, _) =>
        s"Struct($enum,${args.map(_.asString).mkString(",")})"
      case LetFreeExpression.Literal(toLit) =>
        toLit match {
...
sealed abstract class LetFreePattern {
  ...
  def asString: String =
    this match {
      ...
      case LetFreePattern.Literal(toLit) =>
        toLit match {
          case Lit.Str(toStr)      => s"Literal('${escapeString(toStr)}')"
          case Lit.Integer(bigInt) => s"Literal($bigInt)"
        }
      ...
      case LetFreePattern.PositionalStruct(name, params, df) =>
        s"PositionalStruct(${name.map(_.toString).getOrElse("")},${params.map(_.asString).mkString(",")})"
      case LetFreePattern.Union(head, rest) =>
        s"Union(${head.asString},${rest.toList.map(_.asString).mkString(",")})"
...
object LetFreePattern {
      ...
      patterns match {
        case head :: rest =>
          head match {
            ...
            case LetFreePattern.PositionalStruct(_, params, df) =>
              loop(floor, params ++ rest)
  ...
  case class ListPat(parts: List[ListPart]) extends LetFreePattern {
    lazy val toMatchList: (
        List[LetFreePattern],
        List[(ListPart.Glob, List[LetFreePattern])]
    ) = {
      def loop(
          parts: List[ListPart]
      ): (List[LetFreePattern], List[(ListPart.Glob, List[LetFreePattern])]) =
        parts match {
          case Nil => (Nil, Nil)
          case Left(None) :: Right(WildCard) :: tail =>
            loop(Right(WildCard) :: Left(None) :: tail)
          case Left(glob) :: tail =>
            loop(tail) match {
  ...
  case class PositionalStruct(
      name: Option[Int],
      params: List[LetFreePattern],
      dataFamily: DataFamily
  ) extends LetFreePattern
  case class Union(head: LetFreePattern, rest: NonEmptyList[LetFreePattern])
      extends LetFreePattern
...
object LetFreeConversion {
  type LetFreeExpressionTag = LetFreeExpression
  ...
  def listAsStructList(lst: List[LetFreeExpression]): LetFreeExpression =
    lst.foldRight(LetFreeExpression.Struct(0, Nil, DataFamily.Enum)) {
      case (lfe, acc) =>
        LetFreeExpression.Struct(1, List(lfe, acc), DataFamily.Enum)
    }
  ...
  val neToStruct: (LetFreeExpression, DataFamily) => Option[
    (Int, List[LetFreeExpression])
  ] = {
    case (LetFreeExpression.Struct(enum, args, _), df) => Some((enum, args))
    case _                                             => None
  }
  val neToList: LetFreeExpression => Option[List[LetFreeExpression]] =
    structListAsList(_)
  val neFromList: List[LetFreeExpression] => LetFreeExpression =
    listAsStructList(_)
  ...
  abstract class MaybeBind[T](pat: LetFreePattern) {
    ...
    val apply: (T, PatternEnv[T]) => PatternMatch[PatternEnv[T]] = pat match {
      ...
      case LetFreePattern.Literal(lit) => { (v, env) =>
        toLitValue(v) match {
          case Some(lv) => if (lv.equivToLit(lit)) Matches(env) else NoMatch
          case _        => NotProvable
        }
      }
      case LetFreePattern.Var(n) => { (v, env) => Matches(env + (n -> v)) }
      ...
      case lp @ LetFreePattern.ListPat(_) => {
        ...
        def loop(
            matchList: NonEmptyList[(ListPart.Glob, List[LetFreePattern])]
      ...
      case LetFreePattern.StrPat(parts) =>
        val listParts: List[LetFreePattern.ListPart] = parts.toList.flatMap {
          ...
          case LetFreePattern.StrPart.LitStr(str) =>
            str.toList.map(c =>
              Right(LetFreePattern.Literal(Lit.Str(c.toString)))
            )
        }
  ...
  case class LetFreeExpressionMaybeBind(pat: LetFreePattern)
      extends MaybeBind[LetFreeExpression](pat) {
    def toLitValue(t: LetFreeExpression): Option[LitValue] = neToLitValue(t)
    def toStruct(
        t: LetFreeExpression,
        df: DataFamily
    ): Option[(Int, List[LetFreeExpression])] = neToStruct(t, df)
    def toList(t: LetFreeExpression): Option[List[LetFreeExpression]] =
      neToList(t)
    def fromList(lst: List[LetFreeExpression]): LetFreeExpression = neFromList(
      lst
    )
    def fromString(str: String): LetFreeExpression =
      LetFreeExpression.Literal(Lit.Str(str))
    def maybeBind(
        pat: LetFreePattern
    ): (LetFreeExpression, PatternEnv[LetFreeExpression]) => PatternMatch[
      PatternEnv[LetFreeExpression]
    ] = LetFreeExpressionMaybeBind(pat).apply(_, _)
  }
  ...
  def solveMatch(
      env: PatternEnv[LetFreeExpression],
      result: LetFreeExpression
  ) =
    ((env.size - 1) to 0 by -1)
      .map(
        env.get(_).get
      ) // If this exceptions then somehow we didn't get enough names in the env
  ...
  def normalOrderReduction(expr: LetFreeExpression): LetFreeExpression = {
    ...
    val res = headReduction(expr) match {
      ...
      case Match(arg, branches) =>
        Match(
          normalOrderReduction(arg),
          branches.map { case (p, s) => (p, normalOrderReduction(s)) }
        )
      ...
      case Struct(enum, args, df) =>
        Struct(enum, args.map(normalOrderReduction(_)), df)
      case l @ Literal(_)       => l
  ...
  def headReduction(expr: LetFreeExpression): LetFreeExpression = {
    ...
    val nextExpr = expr match {
      ...
      case Recursion(Lambda(innerExpr))
          if (innerExpr.maxLambdaVar.map(_ < 0).getOrElse(true)) =>
  ...
  private def applyLambdaSubstituion(
    ...
    expr match {
      case App(fn, arg) =>
        App(
          applyLambdaSubstituion(fn, subst, idx),
      ...
      case Match(arg, branches) =>
        Match(
          applyLambdaSubstituion(arg, subst, idx),
          branches.map { case (enum, expr) =>
            (enum, applyLambdaSubstituion(expr, subst, idx))
          }
        )
      ...
      case Lambda(fn) =>
        Lambda(
          applyLambdaSubstituion(
            fn,
            subst.map(incrementLambdaVars(_, 0)),
            idx + 1
          )
        )
      case Struct(enum, args, df) =>
      ...
      case l @ Literal(_) => l
      case r @ Recursion(fn) =>
        Recursion(applyLambdaSubstituion(fn, subst, idx))
    }
  }

  def incrementLambdaVars(
      expr: LetFreeExpression,
      lambdaDepth: Int
  ): LetFreeExpression = {
    import LetFreeExpression._
    expr match {
      case App(fn, arg) =>
        App(
          incrementLambdaVars(fn, lambdaDepth),
      ...
      case Match(arg, branches) =>
        Match(
          incrementLambdaVars(arg, lambdaDepth),
          branches.map { case (enum, expr) =>
            (enum, incrementLambdaVars(expr, lambdaDepth))
          }
        )
      case LambdaVar(varIndex) if varIndex >= lambdaDepth =>
        LambdaVar(varIndex + 1)
      case lv @ LambdaVar(_) => lv
...
case class LetFreePackageMap(pm: PackageMap.Inferred) {
  ...
  def letFreeConvertExpr(
    ...
    expr match {
      ...
      case al @ AnnotatedLambda(_, _, _, _) =>
  ...
  def letFreeConvertGeneric(
      g: Generic[Declaration],
      env: Env,
      p: Package.Inferred
  ): NormState[TypedExpr[(Declaration, LetFreeExpressionTag)]] =
  ...
  def letFreeConvertGlobal(
      v: Global[Declaration],
      env: Env,
      p: Package.Inferred
  ): NormState[TypedExpr[(Declaration, LetFreeExpressionTag)]] = {
  ...
  def letFreeConvertAnnotatedLambda(
  ...
  def letFreeConvertLiteral(
      l: Literal[Declaration],
      env: Env,
      p: Package.Inferred
  ): NormState[TypedExpr[(Declaration, LetFreeExpressionTag)]] =
    State.pure(l.copy(tag = (l.tag, LetFreeExpression.Literal(l.lit))))

  def letFreeConvertMatch(
      m: Match[Declaration],
      env: Env,
      p: Package.Inferred
  ): NormState[TypedExpr[(Declaration, LetFreeExpressionTag)]] = for {
    ...
    def loop(pat: Pattern[(PackageName, Constructor), Type]): LetFreePattern =
      pat match {
        ...
        case Pattern.Named(n, p) =>
          LetFreePattern.Named(names.indexOf(n), loop(p))
        case Pattern.StrPat(items) =>
          LetFreePattern.StrPat(items.map {
            case Pattern.StrPart.WildStr => LetFreePattern.StrPart.WildStr
            case Pattern.StrPart.NamedStr(n) =>
              LetFreePattern.StrPart.NamedStr(names.indexOf(n))
            case Pattern.StrPart.LitStr(asString) =>
              LetFreePattern.StrPart.LitStr(asString)
          })
        ...
        case Pattern.PositionalStruct(pc @ (_, ctor), params) =>
          ...
          val name =
            if (dt.isStruct) None
            else Some(dt.constructors.indexWhere(_.name == ctor))
          LetFreePattern.PositionalStruct(name, params.map(loop(_)), df)
        case Pattern.Union(h, t) =>
          LetFreePattern.Union(loop(h), t.map(loop(_)))
      }
    loop(pat)
  }

  def letFreeConvertBranch(
      b: (Pattern[(PackageName, Constructor), Type], TypedExpr[Declaration]),
      env: Env,
      p: Package.Inferred
  ): NormState[
    (
    ...
    val lv = names.zipWithIndex.reverse.toMap
      .mapValues(idx => LetFreeExpression.LambdaVar(idx))

    val nextEnv = lv ++ ((env -- names).mapValues { lfe =>
      names.foldLeft(lfe) { (expr, _) =>
        LetFreeConversion.incrementLambdaVars(expr, 0)
      }
    })

    for {
      innerExpr <- letFreeConvertExpr(expr, nextEnv, p)
      normalExpr = names.foldLeft(innerExpr.tag._2) { case (expr, _) =>
        LetFreeExpression.Lambda(expr)
      }
      finalExpression = innerExpr.updatedTag((innerExpr.tag._1, normalExpr))
    } yield (pattern, finalExpression)
  }

  def letFreeConvertProgram(
      pkgName: PackageName,
      pack: Package.Inferred
  ): NormState[Program[TypeEnv[Variance], TypedExpr[
    (Declaration, LetFreeConversion.LetFreeExpressionTag)
  ], Any]] = {
    for {
      lets <- pack.program.lets.map { case (name, recursive, expr) =>
        letFreeConvertNameKindLet(name, recursive, expr, pack, Map.empty).map(
          (name, recursive, _)
        )
      }.sequence
  ...
  private type NormState[A] = State[Map[(PackageName, Identifier), TypedExpr[
  ...
  private def letFreeConvertNameKindLet(
