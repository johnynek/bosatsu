...
final class SourceConverter(
  ...
  private def apply(
    ...
    decl match {
        ...
        def solvePat(
          ...
          pat match {
            ...
            case pat =>
              ...
              (convertPattern(pat, decl.region), erest, rrhs).mapN {
                (newPattern, e, rhs) =>
      ...
      case DefFn(defstmt @ DefStatement(_, _, _, _)) =>
        ...
        // TODO
        val lambda = defstmt.toLambdaExpr(
          { res => withBound(res._1.get, newBindings) },
          success(decl)
        ...
        deep
          ...
          .flatMap { argsType =>
            ...
            updateInferedWithDecl(typeArgs, typeParams0).map { typeParams =>
              ...
              val consFn =
                rankn.ConstructorFn.build(pname, tname, typeParams, nm, params)
      ...
      case ExternalStruct(nm, targs) =>
        ...
        success(
          rankn.DefinedType(
            pname,
            TypeName(nm),
            targs.map { case TypeRef.TypeVar(v) => (Type.Var.Bound(v), ()) },
            Nil
          )
        )
    }
  ...
  def toProgram(
      ss: List[Statement]
  ): Result[Program[(TypeEnv[Variance], ParsedTypeEnv[Unit]), Expr[
    Declaration
  ], List[Statement]]] = {
