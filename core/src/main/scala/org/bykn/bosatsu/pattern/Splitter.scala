...
abstract class Splitter[-Elem, Item, Sequence, R] {
  ...
  def positions(c: Elem): Sequence => Stream[(Sequence, Item, R, Sequence)]
...
object Splitter {
  def stringSplitter[R](fn: Char => R)(implicit m: Monoid[R]): Splitter[Char, Char, String, R] =
    new Splitter[Char, Char, String, R] {
      ...
      val monoidResult = m
      def positions(c: Char): String => Stream[(String, Char, R, String)] = { str =>
        def loop(init: Int): Stream[(String, Char, R, String)] =
      ...
      def anySplits(str: String): Stream[(String, Char, R, String)] =
        (0 until str.length)
          .toStream
          .map { idx =>
  ...
  abstract class ListSplitter[P, V, R] extends Splitter[P, V, List[V], R] {
    final def positions(c: P): List[V] => Stream[(List[V], V, R, List[V])] = {
      val matchFn = matcher(c)

      { str: List[V] =>
        def loop(tail: List[V], acc: List[V]): Stream[(List[V], V, R, List[V])] =
          tail match {
    ...
    final def anySplits(str: List[V]): Stream[(List[V], V, R, List[V])] = {
      def loop(str: List[V], acc: List[V]): Stream[(List[V], V, R, List[V])] =
        str match {
          case Nil => Stream.Empty
