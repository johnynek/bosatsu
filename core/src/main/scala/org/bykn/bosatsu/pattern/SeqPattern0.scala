...
sealed trait SeqPattern0[+A] {
  import SeqPattern0._
  ...
  /**
   * Concat that SeqPattern0 on the right
   */
  def +[A1 >: A](that: SeqPattern0[A1]): SeqPattern0[A1] =
    SeqPattern0.fromList(toList ::: that.toList)

  def reverse: SeqPattern0[A] =
    SeqPattern0.fromList(toList.reverse)

  def prependWild: SeqPattern0[A] =
    this match {
...
object SeqPattern0 {
  case object Empty extends SeqPattern0[Nothing]
  case class Cat[A](head: SeqPart[A], tail: SeqPattern0[A]) extends SeqPattern0[A] {
  ...
  def fromList[A](ps: List[SeqPart[A]]): SeqPattern0[A] =
    ps match {
      case h :: tail =>
        Cat(h, fromList(tail))
      case Nil => Empty
  ...
  val Wild: SeqPattern0[Nothing] = Cat(SeqPart.Wildcard, Empty)
  val Any: SeqPattern0[Nothing] = Cat(SeqPart.AnyElem, Empty)

  implicit def ordSeqPattern0[A: Ordering]: Ordering[SeqPattern0[A]] =
    new Ordering[SeqPattern0[A]] {
      val ordSeqPart: Ordering[SeqPart[A]] = implicitly[Ordering[SeqPart[A]]]
      def compare(a: SeqPattern0[A], b: SeqPattern0[A]) =
        (a, b) match {
          case (Empty, Empty) => 0
  ...
  implicit def seqPatternSetOps[A](implicit part1SetOps: SetOps[SeqPart.SeqPart1[A]], ordA: Ordering[A]): SetOps[SeqPattern0[A]] =
    new SetOps[SeqPattern0[A]] {
      import SeqPart.{SeqPart1, Lit, AnyElem, Wildcard}

      lazy val top = Some(Wild)
      def isTop(p: SeqPattern0[A]) = p.matchesAny
      ...
      def unifyUnion(union: List[SeqPattern0[A]]): List[SeqPattern0[A]] =
        unifyUnionList {
            union
              .map(_.normalize)
              .distinct
          ...
          .map(SeqPattern0.fromList(_).normalize)
          .sorted
       ...
       */
      def subset(p1: SeqPattern0[A], p2: SeqPattern0[A]): Boolean =
       ...
       */
      def intersection(p1: SeqPattern0[A], p2: SeqPattern0[A]): List[SeqPattern0[A]] =
        (p1, p2) match {
          case (Empty, _) =>
       ...
       */
      def difference(p1: SeqPattern0[A], p2: SeqPattern0[A]): List[SeqPattern0[A]] =
        (p1, p2) match {
