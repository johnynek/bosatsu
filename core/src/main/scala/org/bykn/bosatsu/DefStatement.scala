...
case class DefStatement[A, B](
  ...
  /** This ignores the name completely and just returns the lambda expression
    * here
    */
  def toLambdaExpr[F[_]: Applicative, C](
      resultExpr: B => F[Expr[C]],
      tag: F[C]
  )(
      argFn: A => F[Pattern[(PackageName, Constructor), rankn.Type]],
      trFn: TypeRef => F[rankn.Type]
  ): F[Expr[C]] = {
...
object DefStatement {
    ...
    Document.instance[DefStatement[A, B]] { defs =>
      ...
      val argDoc =
        Doc.char('(') +
          ...
          Doc.char(')')
      val line0 =
        defDoc + Document[Bindable].document(name) + argDoc + res + Doc.char(
          ':'
        )
  ...
  def parser[A, B](
    ...
    (
      Parser.keySpace(
        "def"
      ) *> (Identifier.bindableParser ~ args) <* maybeSpace,
