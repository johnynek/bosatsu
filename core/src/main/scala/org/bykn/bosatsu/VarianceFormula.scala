  ...
  def substitute(uk: Unknown, v: Variance): VarianceFormula =
    this match {
      ...
      case Times(left, right) =>
        left.substitute(uk, v) * right.substitute(uk, v)
...
object VarianceFormula {
  ...
  private object Identifier {
    ...
    val nextIdent: State[SolutionState, Identifier] =
      State { (ss: SolutionState) =>
        require(
          ss.next.toLong < Long.MaxValue,
          "we shouldn't run long enough to overflow Long"
        )
  ...
  private case class SolutionState(
    ...
    /** After we have processed all the constraints, then all the remaining
      * unknowns must be Phantom
      */
    ...
    /** remove any constraints we can solve for. This is the main meat of the
      * algorithm
      */
    def simplify: SolutionState = {
      ...
      def loop(ss: SolutionState): SolutionState = {
        ...
        val solved = ss.constraints.iterator.collect { case (u, Known(v)) =>
          (u, v)
        }.toMap
        if (solved.isEmpty) {
          if (!ss.isSolved) {
            val (solves, _, finish) =
              iteration(
                ss.constraints.view.mapValues(_ => Phantom).toMap,
                ss.constraints,
                ss.constraints.size + 100
              )
          ...
          val newConstraints = solved.foldLeft(ss.constraints) {
            case (cons, (u, v)) =>
              cons.iterator.collect {
      ...
      @annotation.tailrec
      def iteration(
          m: Map[Unknown, Variance],
          constraints: Map[Unknown, VarianceFormula],
          trials: Int
      ): (Map[Unknown, Variance], Map[Unknown, VarianceFormula], Boolean) = {
  ...
  private object SolutionState {
    val empty: SolutionState =
      SolutionState(Identifier.init, Map.empty, Map.empty, Set.empty)
  }

  case class Known(toVariance: Variance) extends VarianceFormula
  case class Unknown(ident: Identifier) extends VarianceFormula
  case class Times(left: VarianceFormula, right: VarianceFormula)
      extends VarianceFormula {
    require(
      !(left.isKnown && right.isKnown),
      s"both $left and $right cannot be known"
    )
  }
  case class Plus(left: VarianceFormula, right: VarianceFormula)
      extends VarianceFormula {
    require(
      !(left.isKnown && right.isKnown),
      s"both $left and $right cannot be known"
  ...
  private val newUnknown: State[SolutionState, Unknown] =
    Identifier.nextIdent.flatMap { id =>
      val u = Unknown(id)
      State { case s @ SolutionState(_, _, _, unknowns) =>
        (s.copy(unknowns = unknowns + u), u)
      }
    }

  /** Solve the variance of the given list of current items or return a list of
    * of those items we cannot infer the variance of.
    *
    * It may be the case that we can always infer the variance, we don't
    * actually have a proof on how often this fails (if ever).
    */
  def solve(
      imports: TypeEnv[Variance],
      current: List[DefinedType[Unit]]
  ): Either[NonEmptyList[DefinedType[Unit]], List[DefinedType[Variance]]] = {
    val travListDT = Traverse[NonEmptyList].compose[DefinedType]

    def constrain(
        unknowns: SortedMap[(PackageName, TypeName), DefinedType[Unknown]],
        dt: DefinedType[Unknown]
    ): State[SolutionState, Unit] = {
      import Type._

      val unit: State[SolutionState, Unit] = State.pure(())

      def constrainUnknown(
          u: Unknown,
          f: VarianceFormula
      ): State[SolutionState, Unit] =
       ...
       */
      def constrainHKinParam(
          tpe: Type,
          umap: Map[Var, Unknown]
      ): State[SolutionState, Unit] =
        tpe match {
    ...
    def finish(
        te: NonEmptyList[DefinedType[Unknown]],
        ss: SolutionState
    ): Either[NonEmptyList[DefinedType[Unit]], List[DefinedType[Variance]]] = {
