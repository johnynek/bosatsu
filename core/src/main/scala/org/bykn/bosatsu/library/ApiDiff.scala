...
import org.bykn.bosatsu.rankn.{ConstructorFn, DefinedType, Type, TypeEnv}
...
object ApiDiff {
  ...
  case class ChangedType(pack: PackageName, oldTpe: Type, newTpe: Type) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc =
        Doc.text(show"in package $pack original type is ") + Type.fullyResolvedDocument.document(oldTpe) + Doc.text(" and new type is ") +
          Type.fullyResolvedDocument.document(newTpe)
  }

  case class ChangedKind(pack: PackageName, oldDt: DefinedType[Kind.Arg], newDt: DefinedType[Kind.Arg]) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc =
        Type.fullyResolvedDocument.document(oldDt.toTypeTyConst) + Doc.text(" changes from kind ") +
        Document[Kind].document(oldDt.kindOf) + Doc.text(" to kind ") +
        Document[Kind].document(newDt.kindOf)
  }

  case class ChangedTransitiveType(tpe: Type, transitive: Type.TyConst, diff: Diff) extends Diff {
    def pack = diff.pack
    def isValidWhen(dk: Version.DiffKind) = diff.isValidWhen(dk)
    def toDoc = {
      def tdoc(t: Type): Doc = Type.fullyResolvedDocument.document(t)

      Doc.text(show"change in the type ") + tdoc(tpe) + Doc.text(" that reaches type ") + tdoc(transitive) +
        Doc.char('.') + (Doc.line + diff.toDoc).nested(4).grouped 
    }
  }

  case class ChangedValue(bindable: Identifier.Bindable, diff: Diff) extends Diff {
    def pack = diff.pack
    def isValidWhen(dk: Version.DiffKind) = diff.isValidWhen(dk)
  ...
  case class ConstructorRemoved(pack: PackageName, tycons: Type.TyConst, cfn: ConstructorFn) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text("removed constructor of ") + Type.fullyResolvedDocument.document(tycons) + Doc.text(show" ${cfn.name}")
  }

  case class ConstructorAdded(pack: PackageName, tycons: Type.TyConst, cfn: ConstructorFn) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text("added constructor of ") + Type.fullyResolvedDocument.document(tycons) + Doc.text(show" ${cfn.name}")
  }

  case class ConstructorIndexChange(pack: PackageName, tycons: Type.TyConst, name: Identifier.Constructor, oldIdx: Int, newIdx: Int) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text(show"constructor named $name of ") + Type.fullyResolvedDocument.document(tycons) +
      Doc.text(show" changed index from $oldIdx to $newIdx")
  }

  case class ConstructorParamChange(name: Identifier.Constructor, tycons: Type.TyConst, paramIndex: Int, diff: Diff) extends Diff {
    def pack = diff.pack
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text(show"constructor named $name of ") + Type.fullyResolvedDocument.document(tycons) +
  ...
  case class ConstructorParamNameChange(pack: PackageName, name: Identifier.Constructor, tycons: Type.TyConst, paramIndex: Int, oldName: Identifier.Bindable, newName: Identifier.Bindable) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text(show"constructor named $name of ") + Type.fullyResolvedDocument.document(tycons) +
      Doc.text(show" at parameter index $paramIndex changed parameter name from $oldName to $newName")
  }

  case class ConstructorParamAdded(pack: PackageName, cons: Identifier.Constructor, tycons: Type.TyConst, paramIndex: Int, name: Identifier.Bindable, tpe: Type) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text(show"constructor named $cons of ") + Type.fullyResolvedDocument.document(tycons) +
      Doc.text(show" at parameter index $paramIndex added parameter $name with type ") +
      Type.fullyResolvedDocument.document(tpe)
  }

  case class ConstructorParamRemoved(pack: PackageName, cons: Identifier.Constructor, tycons: Type.TyConst, paramIndex: Int, name: Identifier.Bindable, tpe: Type) extends Diff {
    def isValidWhen(dk: Version.DiffKind) = dk.isMajor
    def toDoc = Doc.text(show"constructor named $cons of ") + Type.fullyResolvedDocument.document(tycons) +
      Doc.text(show" at parameter index $paramIndex removed parameter $name with type ") +
      Type.fullyResolvedDocument.document(tpe)
  }
  ...
  private def apply(pn: PackageName, prev: List[ExportedName[R]], prevEnv: TypeEnv[Kind.Arg], current: List[ExportedName[R]], curEnv: TypeEnv[Kind.Arg]): List[Diff] = {
    ...
    def diffType(oldTpe: Type, newTpe: Type): List[Diff] =
      ...
      if (oldTpe.sameAs(newTpe)) {
        ...
        consts.flatMap { const =>
          val oldDt = prevEnv.getType(const).getOrElse(sys.error(s"tpe=$oldTpe prevEnv=$prevEnv doesn't have $const"))
          val newDt = curEnv.getType(const).getOrElse(sys.error(s"tpe=$oldTpe prevEnv=$curEnv doesn't have $const"))
          diffDT(oldDt, newDt)
    ...
    def diffDT(oldDt: DefinedType[Kind.Arg], newDt: DefinedType[Kind.Arg]): List[Diff] = {
                  ...
                  oldCfn.args
                    ...
                    .flatMap {
                      case (Ior.Both((oldName, oldType), (newName, newType)), idx) =>
                        val tpeDiff = diffType(oldType, newType).map(ConstructorParamChange(cons, oldDt.toTypeTyConst, idx, _))

                        if (oldName == newName) tpeDiff
                        else (ConstructorParamNameChange(pn, cons, oldDt.toTypeTyConst, idx, oldName, newName) :: tpeDiff)
                      case (Ior.Right((newName, newType)), idx) =>
                        ConstructorParamAdded(pn, cons, newDt.toTypeTyConst, idx, newName, newType) :: Nil
        ...
        case (Some(oldExp), Some(newExp)) =>
          name match {
            ...
            case cons: Identifier.Constructor =>
              def typesOf(nel: NonEmptyList[ExportedName[R]]): List[DefinedType[Kind.Arg]] =
                nel.toList.collect {
                  case ExportedName.TypeName(_, Referant.DefinedT(dt)) => dt
                  case ExportedName.Constructor(_, Referant.Constructor(dt, _)) => dt
                }
                .distinct
