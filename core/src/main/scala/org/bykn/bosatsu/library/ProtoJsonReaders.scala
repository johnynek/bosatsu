...
object ProtoJsonReaders {
  ...
  implicit val descriptorReader: Reader[proto.LibDescriptor] =
    new Reader.Obj[proto.LibDescriptor] {
      ...
      def readObj(
          from: Reader.FromObj
      ): Either[(String, Json, Json.Path), proto.LibDescriptor] =
        for {
  ...
  implicit val descriptorWriter: Writer[proto.LibDescriptor] =
    Writer.from[proto.LibDescriptor] { ld =>
      Json.JObject(
        (ld.version match {
          case Some(v) =>
            ("version" -> Json.JString(Version.fromProto(v).render)) :: Nil
          case None => Nil
        }) :::
  ...
  implicit val historyReader: Reader[proto.LibHistory] =
    new Reader.Obj[proto.LibHistory] {
      ...
      def readObj(
          from: Reader.FromObj
      ): Either[(String, Json, Json.Path), proto.LibHistory] =
        for {
          ...
          previousPre <- from.optional[proto.LibDescriptor](
            "previous_prerelease"
          )
    ...
    Writer.from { (hist: proto.LibHistory) =>
      ...
      Json.JObject(
        (hist.previousMajor.toList.map(d => "previous_major" -> write(d))) :::
          ...
          (hist.previousPrerelease.toList
            .map(d => "previous_prerelease" -> write(d))) :::
          (if (hist.others.isEmpty) Nil
           else ("others" -> write(hist.others.toList) :: Nil)) :::
          Nil
      )
    }
