...
object Declaration {
  ...
  case class LeftApply(
    ...
    def rewrite: NonBinding = {
      val lam = Lambda(NonEmptyList(arg, Nil), result.padded)(
  ...
  def bindingLike[S](
      parser: Indy[NonBinding],
      sym: P[S],
      cutPattern: Boolean
  ): Indy[(Pattern.Parsed, Region, S, NonBinding)] = {
    ...
    OptIndent
      .blockLike(Indy.lift(cutOrNot), parser, P.unit)
  ...
  def lambdaP(parser: Indy[Declaration]): Indy[Lambda] = {
    ...
    OptIndent
      .blockLike(params, parser, maybeSpace.with1 *> P.string("->"))
      .region
  ...
  def recordConstructorP(
    ...
    val tupArgs = declP.parensLines1Cut.region
      .map {
        case (r, args) => { (nm: Var) =>
          Apply(nm, args, ApplyKind.Parens)(nm.region + r)
        }
      }
      ...
      case ((pm, indent), rec) =>
        ...
        val applied: P[NonBinding] = {
          ...
          val dotApply: P[NonBinding => NonBinding] =
            (slashcontinuation.with1 *> P.char('.') *> (fn ~ params.?)).region
            ...
            params.region
              .map {
                case (r, args) => { (fn: NonBinding) =>
                  Apply(fn, args, ApplyKind.Parens)(fn.region + r)
                }
              }
