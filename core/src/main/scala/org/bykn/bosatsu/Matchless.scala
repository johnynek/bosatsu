...
object Matchless {
  ...
  def fromLet[F[_]: Monad, A](
    ...
    def doesMatch(arg: CheapExpr, pat: Pattern[(PackageName, Constructor), Type], mustMatch: Boolean): F[UnionMatch] = {
      pat match {
                    ...
                    .map { cases =>
                      cases.map {
                        ...
                        case (preLet, expr, binds) =>
                          ...
                          (resLet, SearchList(anonList, arg, expr, leftOpt), resBind)
                      }
                    }
                }
              ...
              maybeSimple(pat) match {
                ...
                case None =>
                  ...
                  for {
                    ...
                    lam = LocalAnonMut(nm)
                    um <- WriterT.valueT[F, Locals, UnionMatch](doesMatch(lam, pat, mustMatch))
            ...
            asStruct { pos => GetStructElement(arg, pos, size) }
              ...
              .map { case (anons, ums) =>
                ums.map { case (pre, cond, bind) =>
                  val pre1 = anons.foldLeft(pre) { case (pre, (a, _)) => a :: pre }
                  // we have to set these variables before we can evaluate the condition
                  val cond1 = anons.foldLeft(cond) { case (c, (a, e)) => SetMut(a, e) && c }
                  (pre1, cond1, bind)
                }
              }
            ...
            case Some(dr) =>
              dr match {
                ...
                case DataRepr.Enum(vidx, size, f) =>
                  ...
                  val cv: BoolExpr = if (mustMatch) TrueConst else CheckVariant(arg, vidx, size, f)
                  asStruct { pos => GetEnumElement(arg, vidx, pos, size) }
                    .run
                    .map { case (anons, ums) =>
                      if (ums == wildMatch) {
                        ...
                        val cond1 = anons.foldLeft(cv) { case (c, (mut, expr)) =>
                          c && SetMut(mut, expr)
                        }
        ...
        case Pattern.Union(h, ts) =>
          ...
          val unionMustMatch = NonEmptyList.fromListUnsafe(List.fill(ts.size)(false) ::: mustMatch :: Nil)
          ((h :: ts).zip(unionMustMatch)).traverse { case (p, mm) => doesMatch(arg, p, mm) }.map { nene =>
    ...
    def matchExpr(arg: Expr, tmp: F[Long], branches: NonEmptyList[(Pattern[(PackageName, Constructor), Type], Expr)]): F[Expr] = {
      ...
      def recur(arg: CheapExpr, branches: NonEmptyList[(Pattern[(PackageName, Constructor), Type], Expr)]): F[Expr] = {
        ...
        def loop(cbs: NonEmptyList[(List[LocalAnonMut], BoolExpr, List[(Bindable, Expr)])]): F[Expr] =
          cbs match {
