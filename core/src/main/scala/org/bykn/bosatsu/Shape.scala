...
import org.bykn.bosatsu.rankn.{ConstructorFn, DefinedType, Type, TypeEnv, Ref, RefSpace}
...
object Shape {
  ...
  case class Cons(arg: Shape, res: Shape) extends Shape {
    require(!(arg.isInstanceOf[KnownShape] && res.isInstanceOf[KnownShape]), s"cons must have one unknown: $this")
  }
  case class Unknown(bound: Either[rankn.Type.TyApply, rankn.Type.Var.Bound], ref: Ref[Option[Shape]]) extends Shape

  def shapeDoc(s: Shape): Doc =
    s match {
      case Type => Doc.char('*')
      case Cons(Type, s2) => Doc.text("* -> ") + shapeDoc(s2)
      case Cons(s1, s2) => Doc.char('(') + shapeDoc(s1) + Doc.char(')') + Doc.text(" -> ") + shapeDoc(s2)
      case KnownCons(Type, s2) => Doc.text("* -> ") + shapeDoc(s2)
      case KnownCons(s1, s2) => Doc.char('(') + shapeDoc(s1) + Doc.char(')') + Doc.text(" -> ") + shapeDoc(s2)
  ...
  case class ShapeLoop(dt: DefinedType[Option[Kind.Arg]],
    bound: Either[rankn.Type.TyApply, rankn.Type.Var.Bound],
    path: Set[Shape]
  ) extends Error

  case class UnificationError(dt: DefinedType[Option[Kind.Arg]],
    cfn: ConstructorFn,
    left: Shape,
    right: Shape
  ) extends Error

  case class UnknownConst(dt: DefinedType[Option[Kind.Arg]],
    cfn: ConstructorFn,
    const: rankn.Type.Const
  ) extends Error

  case class UnboundVar(dt: DefinedType[Option[Kind.Arg]],
    cfn: ConstructorFn,
    bound: rankn.Type.Var.Bound
  ) extends Error

  case class ShapeMismatch(dt: DefinedType[Option[Kind.Arg]],
    cfn: ConstructorFn,
    outer: rankn.Type,
  ...
  object ShapeOf {
    implicit val kindShapeOf: ShapeOf[Kind] =
      { (k: Kind) => shapeOf(k) }

    implicit val kindArgShapeOf: ShapeOf[Kind.Arg] =
      { (k: Kind.Arg) => shapeOf(k.kind) }

    implicit val knownShapeOf: ShapeOf[KnownShape] =
      { (k: KnownShape) => k }

    implicit def definedTypeShapeOf[S: ShapeOf]: ShapeOf[DefinedType[S]] =
      { (dt: DefinedType[S]) =>
        dt.annotatedTypeParams
          .foldRight(Type: KnownShape) { (s, ks) => KnownCons(ShapeOf(s._2), ks) }
      }

    implicit def eitherShapeOf[A: ShapeOf, B: ShapeOf]: ShapeOf[Either[A, B]] =
      { (e: Either[A, B]) =>
        e match {
  ...
  def solveShape[S: ShapeOf](
    ...
    def thisScope(lst: List[(rankn.Type.Var.Bound, Either[Shape, Kind.Arg])]): Env = {
        ...
        cache.getOrElseUpdate(t, {
          t match {
            ...
            case rankn.Type.TyConst(const) =>
              imports.toDefinedType(const).map { dt => ShapeOf(dt) }
            case _ =>
    ...
    def shapeToKnown(s: Shape): RefSpace[ValidatedNec[Error, KnownShape]] = {
      def toKnown(s: Shape, path: Set[Shape], allUK: Boolean): RefSpace[ValidatedNec[Error, KnownShape]] =
        s match {
              ...
              case Some(s) =>
                ...
                if (nextPath(s)) {
                  ...
                  if (allUK) {
                    ...
                    RefSpace.pure(Validated.invalidNec(ShapeLoop(dt, b, nextPath)))
                  }
                }
                else {
    ...
    def unifyCons(cfn: ConstructorFn, a: Shape, b: Shape, c: Shape, d: Shape): RefSpace[ValidatedNec[Error, Unit]] =
      (unifyShape(cfn, a, c), unifyShape(cfn, b, d)).mapN(_ *> _)

    def unifyShape(cfn: ConstructorFn, s1: Shape, s2: Shape): RefSpace[ValidatedNec[Error, Unit]] = {
      @inline def error = RefSpace.pure(Validated.invalidNec(UnificationError(dt, cfn, s1, s2)))
      s1 match {
        case Type =>
          s2 match {
            case Type => pureUnit
    ...
    def applyShape(cfn: ConstructorFn, outer: rankn.Type, inner: rankn.Type.TyApply, s1: Shape, s2: Shape): RefSpace[ValidatedNec[Error, Shape]] =
      s1 match {
        case Type =>
          // kind mismatch
          RefSpace.pure(Validated.invalidNec(ShapeMismatch(dt, cfn, outer, inner, s1, s2)))
        case Cons(arg, res) =>
          // s1 == (arg -> res)
          // s1(s2) => res
        ...
        case u1 @ Unknown(_, _) =>
          ...
          RefSpace.newRef(Option.empty[Shape])
            .flatMap { ref =>
    ...
    def shapeOfType(cfn: ConstructorFn, scope: Env, tpe: rankn.Type): RefSpace[ValidatedNec[Error, Shape]] = {
      def loop(inner: rankn.Type, local: Map[rankn.Type.Var.Bound, Kind]): RefSpace[ValidatedNec[Error, Shape]] = 
        inner match {
          ...
          case const @ rankn.Type.TyConst(c) =>
            scope(const) match {
              ...
              case None =>
                // Unknown type name
                RefSpace.pure(Validated.invalidNec(
    ...
    def combineError[F[_]: cats.Foldable](errs: F[ValidatedNec[Error, Unit]]): ValidatedNec[Error, Unit] =
      errs.foldLeft(Validated.valid(()): ValidatedNec[Error, Unit])(_ *> _)

    def constrainFn(scope: Env, cfn: ConstructorFn): RefSpace[ValidatedNec[Error, Unit]] =
      cfn.args.traverse { case (_, tpe) => shapeOfType(cfn, scope, tpe) }
        .flatMap {
          _.sequence match {
            case Validated.Valid(shapeList) =>
              // all the args to a function arg kind Type
              shapeList.traverse(unifyShape(cfn, _, Type))
                .map(combineError(_))
    ...
    def constrainAll(scope: Env): RefSpace[ValidatedNec[Error, Unit]] =
      dt.constructors.traverse(constrainFn(scope, _))
    ...
    val rf = for {
      ...
      params1: List[ValidatedNec[Error, (rankn.Type.Var.Bound, Either[KnownShape, Kind.Arg])]] <-
        shapes.traverse {
          case (v, Left(s)) =>
            shapeToKnown(s)
    ...
    } yield check *> params1.sequence.map { p => dt.copy(annotatedTypeParams = p) }
