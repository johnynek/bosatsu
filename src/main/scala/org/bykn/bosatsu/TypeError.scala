...
object TypeError {
  ...
  case class NonTotalMatch(matches: NonEmptyList[ConstructorName], expected: List[ConstructorName]) extends TypeError
  case class InsufficientPatternBind(cname: ConstructorName, params: List[Option[String]], expectedTypes: List[Type], in: DefinedType) extends TypeError
}
