...
5. Keep only the local package closure required by those exported packages.
6. Lock the resulting allowlist in tests.
7. Treat the shrink as a breaking change and version it accordingly.
...
The current source tree adds three more packages that must remain exported for the seed set to stay valid:
...
- `Bosatsu/Num/Nat`
  Reason: exported JSON parsing and rendering helpers use `Nat`-based fuel and `to_Nat`.
...
- `Bosatsu/Num/BinNat`, `Bosatsu/Num/Binary`, `Bosatsu/Dict`, `Bosatsu/Collection/Queue`, `Bosatsu/Collection/TreeList`, `Bosatsu/Nothing`
  Reason: they are higher-level pure packages, not compiler-known, command-known, or runtime-backed surface required by the seed set.
...
This reduces the public surface from 23 packages to 12 while preserving the compiler/runtime contract and the command-facing `Json` and standard-I/O APIs.

## Implementation Plan

1. Update `test_workspace/core_alpha_conf.json`.
   - Replace the current `exported_packages` list with the 12-package allowlist above.
...
2. Add a regression test for the allowlist.
   ...
   - Include an assertion comment or note that `Bosatsu/Json` is command-known and `Bosatsu/IO/Std` is intentionally retained public surface, so future cleanup does not remove them accidentally.
...
3. Add an interface-visibility integration test.
   ...
   - Verify a downstream source cannot import a removed package such as `Bosatsu/Dict` or `Bosatsu/Collection/Queue` through the dependency interface.
...
- `test_workspace/core_alpha_conf.json` exports exactly the 12-package allowlist documented above.
