...
Updated the existing user/developer documentation to accurately present the shipped `Bosatsu/IO/Core` direct-child lifecycle APIs and managed `with_process` helper, including stable status observation, stop-operation limits, explicit stdio ownership, and a type-correct grace-`Duration` example. The current supervised gate did not reach repository tests: `.mergexo/context/required-tests/c93af409609aa5946311563d5dc6d946db4c1e8a/01a09354520877219479c168bc47cde8/report.json` records exit 127 at tested HEAD `c93af409609aa5946311563d5dc6d946db4c1e8a` because `scripts/test_basic.sh` could not find `sbt`; stdout is empty and stderr contains only that missing-command error. The candidate changes only documentation and its code-plan artifacts and does not modify the required-test script or build tooling. This worker-provisioning failure demonstrates no repository defect, warrants no additional pending step for this documentation-only issue, and should be handled by MergeXO's automatic gate rerun in an environment with `sbt` available.
