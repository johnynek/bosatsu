...
Updated the existing user/developer documentation to accurately present the shipped `Bosatsu/IO/Core` direct-child lifecycle APIs and managed `with_process` helper, including stable status observation, stop-operation limits, explicit stdio ownership, and a type-correct grace-`Duration` example.

No repository repair is warranted for the final required-tests failure. The current diagnostic evidence, `.mergexo/context/required-tests/0956fa74f1b3cb4ef15a9df05d1882ee8a50d5ae/01a0935aca7970f3a6be290948df05be/report.json`, records exit 127 after 0.016 seconds at tested HEAD `0956fa74f1b3cb4ef15a9df05d1882ee8a50d5ae`; stdout is empty and stderr contains only `scripts/test_basic.sh: line 12: sbt: command not found`. The staged attempt index and complete stream hashes show the identical pre-test environment failure at all six tested candidate heads, including the documentation commit and five subsequent plan-only revisions. The candidate changes only documentation and code-plan artifacts, while `scripts/test_basic.sh` and build configuration are unchanged from `main`; changing repository documentation, tests, or the gate script would not supply the missing worker dependency. The completed step remains sufficient, no pending step is added, and MergeXO can rerun the supervised gate when `sbt` is available in the worker environment.

Plan trajectory: initial steps 1; review revisions 0; review-origin steps 0.

## Current State

`main` contains the complete process surface in `test_workspace/Bosatsu/IO/Core.bosatsu`: `StopResult`, `terminate`, `kill`, `poll`, `wait_timeout`, stable `wait`, and the error-polymorphic `with_process` helper. The durable contracts in `docs/design/2365-specify-the-portable-process-stop-and-status-contract.md` and `docs/design/2371-specify-the-managed-with-process-helper-contract.md` describe their semantics, and existing coverage in `test_workspace/Bosatsu/IO/ProcessWaitMain.bosatsu` and `c_runtime/test.c` exercises stable observations, timeout non-consumption, stop behavior, low-level pipe ownership, and helper cleanup.

The completed candidate work now updates `docs/src/main/paradox/design-docs/minimal_prog_io_tools_design.md` and `docs/src/main/paradox/design-docs/prog_concurrency_design.md` so both active Paradox pages match that shipped direct-child lifecycle surface. The candidate diff remains documentation-only apart from its code-plan artifacts; it does not change `scripts/test_basic.sh`, build configuration, executable documentation tooling, runtime code, or tests.
