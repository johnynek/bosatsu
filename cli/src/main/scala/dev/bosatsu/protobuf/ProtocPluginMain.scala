...
object ProtocPluginMain {
  ...
  private final case class MessageInfo(
    ...
    val isMapEntry: Boolean =
      descriptor.hasOptions && descriptor.getOptions.getMapEntry
  }
  ...
  def runFromStdio(): Unit = {
    val response =
      try {
        ...
        case NonFatal(err) =>
          CodeGeneratorResponse.newBuilder().setError(err.getMessage).build()
      }
  ...
  private[protobuf] def generateResponse(
    ...
    buildModels(request) match {
      case Left(errors) =>
        CodeGeneratorResponse
          .newBuilder()
          .setError(errors.mkString("\n"))
          .build()

      case Right(models) =>
        val requestedFiles = request.getFileToGenerateList.asScala.toSet
  ...
  private def buildModels(
    ...
    val descriptors = request.getProtoFileList.asScala.toVector
    ...
    if (validations.nonEmpty) Left(validations)
      ...
      val oneofTypeNameByMessage: Map[String, Vector[(Int, String)]] =
        sortedMessages.map { info =>
          ...
          val oneofDecls = info.descriptor.getOneofDeclList.asScala.toVector
          val activeOneofIndexes =
            info.descriptor.getFieldList.asScala.iterator
              .filter(field => field.hasOneofIndex && !field.getProto3Optional)
              .map(_.getOneofIndex)
              .toSet
  ...
  private def validateFile(file: FileDescriptorProto): Vector[String] = {
    ...
    val groupErrors =
      collectMessages(file).flatMap { info =>
        info.descriptor.getFieldList.asScala.toVector.collect {
  ...
  private def collectEnums(file: FileDescriptorProto): Vector[EnumInfo] = {
    ...
    val topLevel = file.getEnumTypeList.asScala.toVector.map { enumDescriptor =>
    ...
    topLevel ++ collectNestedEnums(
      ...
      protoPackage,
      Nil,
      file.getMessageTypeList.asScala.toVector
    )
  }
  ...
  private def collectNestedEnums(
    ...
    messages.flatMap { messageDescriptor =>
      ...
      val thisEnums = messageDescriptor.getEnumTypeList.asScala.toVector.map {
        enumDescriptor =>
      ...
      thisEnums ++ collectNestedEnums(
        ...
        protoPackage,
        messagePath,
        messageDescriptor.getNestedTypeList.asScala.toVector
      )
    }
  ...
  private def convertEnum(
    ...
    val values = enumInfo.descriptor.getValueList.asScala.toVector
  ...
  private def convertMessage(
    ...
    val fields = messageInfo.descriptor.getFieldList.asScala.toVector
    ...
    val regularFields = fields.zipWithIndex.filterNot { case (field, _) =>
      field.hasOneofIndex && realOneofIndexes(field.getOneofIndex)
    }
    ...
    val regularModels = regularFields.map { case (field, idx) =>
      ...
      val inOneof = field.hasOneofIndex && realOneofIndexes(field.getOneofIndex)
    ...
    val oneofDecls = messageInfo.descriptor.getOneofDeclList.asScala.toVector
    ...
    val oneofModels = oneofDecls.zipWithIndex.collect {
      case (oneofDecl, oneofIdx) if realOneofIndexes(oneofIdx) =>
        ...
        val oneofFields = fields.filter(field =>
          field.hasOneofIndex && field.getOneofIndex == oneofIdx &&
    ...
    if (fieldType.equals(FieldDescriptorProto.Type.TYPE_DOUBLE))
    ...
    else if (fieldType.equals(FieldDescriptorProto.Type.TYPE_MESSAGE))
      {
        ...
        if (
          ...
          val mapEntryFields = messageInfo.descriptor.getFieldList.asScala.toVector
  ...
  private def fieldPacked(
    ...
    else if (field.hasOptions && field.getOptions.hasPacked) field.getOptions.getPacked
    else true
