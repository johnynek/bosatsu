...
object ProtoConverter {
        ...
        ex.value match {
          ...
          case Value.GenericExpr(proto.GenericExpr(typeParams, expr, _)) =>
            NonEmptyList.fromList(typeParams.toList) match {
              ...
              case None =>
                Failure(
                  new Exception(s"invalid empty type params in generic: $ex")
                )
            }
              ...
              typeToProto(t).flatMap { idx =>
                bs.toList
                  ...
                  .flatMap { ids =>
                    getTypeId(
                      p,
                      proto.Type(Value.TypeForAll(TypeForAll(ids, idx)))
                    )
                  }
              }
      ...
      case Some(tc) =>
        for {
          ...
          cons <- pdt.constructors.toList.traverse(
            consFromProto(tconst, tparams.map(_._1), _)
          )
  ...
  def expNameToProto(
      allDts: Map[(PackageName, TypeName), (DefinedType[Any], Int)],
      e: ExportedName[Referant[Variance]]
  ...
  ): DTab[ExportedName[Referant[Variance]]] = {
  ...
  def importedNameToProto(
      allDts: Map[(PackageName, TypeName), (DefinedType[Any], Int)],
      in: ImportedName[NonEmptyList[Referant[Variance]]]
  ...
  def packageToProto[A](cpack: Package.Typed[A]): Try[proto.Package] = {
    ...
    val allDts
        : SortedMap[(PackageName, TypeName), (DefinedType[Variance], Int)] =
  ...
  ): DTab[ImportedName[NonEmptyList[Referant[Variance]]]] = {
  ...
  def importsFromProto(
      ...
      loadDT: Type.Const => Try[DefinedType[Variance]]
  ): DTab[Import[Package.Interface, NonEmptyList[Referant[Variance]]]] =
  ...
  ): DTab[Program[TypeEnv[Variance], TypedExpr[Unit], Unit]] =
    ReaderT
      .ask[Try, DecodeState]
      .map { ds =>
      ...
      if (dupNames.nonEmpty) {
        ...
        def makeLoadDT(
            load: String => Try[Either[
              (Package.Interface, TypeEnv[Variance]),
              Package.Typed[Unit]
            ]]
        ): Type.Const => Try[DefinedType[Variance]] = {
        ...
        val load: String => Try[
          Either[(Package.Interface, TypeEnv[Variance]), Package.Typed[Unit]]
        ] =
